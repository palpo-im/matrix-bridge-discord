...
impl AdminNotifier {
    ...
    pub async fn notify(&self, message: &str) -> Result<()> {
        info!("sending admin notification to {}: {}", self.admin_mxid, message);
        
        let room_id = self.ensure_dm_room().await?;
        
        self.matrix_client
            .send_text(&room_id, message)
            .await?;
    ...
    async fn ensure_dm_room(&self) -> Result<String> {
        ...
        if let Some(room_id) = existing {
            debug!("found existing DM room with {}: {}", self.admin_mxid, room_id);
            return Ok(room_id);
        }
        ...
        for room_id in rooms {
            if let Ok(members) = self.matrix_client.get_room_members(&room_id).await {
                let is_dm = members.len() == 2 
                    && members.iter().any(|m| m == &self.admin_mxid);
                if is_dm {
