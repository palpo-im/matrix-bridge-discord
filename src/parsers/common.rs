...
impl ParsedMessage {
    pub fn new(content: &str) -> Self {
        Self {
            ...
            formatted: None,
            mentions: MessageUtils::extract_discord_user_mentions(content),
            channel_mentions: MessageUtils::extract_discord_channel_mentions(content),
            role_mentions: MessageUtils::extract_discord_role_mentions(content),
            emoji_mentions: MessageUtils::extract_discord_emojis(content),
...
impl MessageUtils {
    ...
    pub fn extract_discord_user_mentions(content: &str) -> Vec<String> {
    ...
    pub fn extract_discord_channel_mentions(content: &str) -> Vec<String> {
        let re = Regex::new(r"<#(\d+)>").unwrap();
        re.captures_iter(content)
            .filter_map(|cap| cap.get(1).map(|m| m.as_str().to_string()))
            .collect()
    ...
    pub fn extract_discord_role_mentions(content: &str) -> Vec<String> {
        let re = Regex::new(r"<@&(\d+)>").unwrap();
        re.captures_iter(content)
            .filter_map(|cap| cap.get(1).map(|m| m.as_str().to_string()))
            .collect()
    ...
    pub fn extract_discord_emojis(content: &str) -> Vec<EmojiMention> {
    ...
    pub fn convert_html_to_discord_markdown(html: &str) -> String {
    ...
    fn convert_html_code_blocks(html: &str) -> String {
        let re = Regex::new(r"<pre[^>]*><code(?:\s+class=\"language-([^\"]*)\")?>([^<]*)</code></pre>").unwrap();
    );
        if lang.is_empty() {
            format!("```\n{}\n```", code)
        } else {
    ...
    pub fn convert_matrix_reply_to_discord(body: &str, reply_content: &str, reply_author: &str) -> String {
    ...
    pub fn escape_discord_special_chars(text: &str) -> String {
        text.replace('\\', "\\\\")
    }
...
fn strip_html_tags(html: &str) -> String {
    ...
    entity_re.replace_all(&result, |caps: &regex::Captures| {
