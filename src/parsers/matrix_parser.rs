...
impl MatrixToDiscordConverter {
    ...
    pub fn format_for_discord(&self, message: &str) -> String {
        ...
        result = self.convert_ghost_users_to_discord(&result);
        result = self.convert_ghost_aliases_to_discord(&result);
        result = self.convert_mxclinks_to_discord(&result);
        result
    }

    pub fn format_html_for_discord(&self, html: &str) -> String {
        let mut result = MessageUtils::convert_html_to_discord_markdown(html);
        result = self.format_for_discord(&result);
        result
    }

    fn convert_ghost_users_to_discord(&self, text: &str) -> String {
    ...
    fn convert_ghost_aliases_to_discord(&self, text: &str) -> String {
    ...
    fn convert_mxclinks_to_discord(&self, text: &str) -> String {
        self.mxclink_regex
            .replace_all(text, |caps: &regex::Captures| {
                let alt_text = &caps[1];
                alt_text.to_string()
    ...
    pub async fn convert_message(
        ...
        let (final_content, formatted_content) = if let Some(html) = formatted_body {
            let converted = self.format_html_for_discord(html);
            (converted.clone(), Some(converted))
        } else {
            let converted = self.format_for_discord(&plain);
    ...
    pub fn format_emote(&self, display_name: &str, body: &str) -> String {
        format!("* {} {}", display_name, self.format_for_discord(body))
    }
}
...
mod tests {
    ...
    #[tokio::test]
    async fn converts_ghost_user_to_discord_mention() {
        let converter = make_converter().await;
        let result = converter.format_for_discord("Hello @_discord_123456789:example.org!");
        assert_eq!(result, "Hello <@123456789>!");
    }

    #[tokio::test]
    async fn converts_ghost_alias_to_discord_channel() {
        let converter = make_converter().await;
        let result = converter.format_for_discord("Check #_discord_987654321:example.org");
    ...
    async fn leaves_regular_users_unchanged() {
        let converter = make_converter().await;
        let result = converter.format_for_discord("Hello @alice:example.org!");
    ...
    async fn converts_html_bold_to_markdown() {
        let converter = make_converter().await;
        let result = converter.format_html_for_discord("<strong>bold</strong> text");
    ...
    async fn converts_html_italic_to_markdown() {
        let converter = make_converter().await;
        let result = converter.format_html_for_discord("<em>italic</em> text");
    ...
    async fn converts_html_code_to_markdown() {
        let converter = make_converter().await;
        let result = converter.format_html_for_discord("<code>inline code</code>");
