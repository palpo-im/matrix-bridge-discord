...
use crate::db::manager::MysqlPool;
use crate::db::schema_mysql::{message_mappings, room_mappings, user_mappings};

fn naive_to_utc(value: NaiveDateTime) -> DateTime<Utc> {
    DateTime::from_naive_utc_and_offset(value, Utc)
}

fn utc_to_naive(value: &DateTime<Utc>) -> NaiveDateTime {
    value.naive_utc()
}
...
impl From<DbRoomMapping> for RoomMapping {
    fn from(value: DbRoomMapping) -> Self {
        Self {
            ...
            created_at: naive_to_utc(value.created_at),
            updated_at: naive_to_utc(value.updated_at),
        }
    }
}
...
async fn with_connection<T, F>(pool: MysqlPool, operation: F) -> Result<T, DatabaseError>
where
    T: Send + 'static,
    F: FnOnce(&mut MysqlConnection) -> Result<T, DatabaseError> + Send + 'static,
{
...
#[async_trait]
impl super::RoomStore for MysqlRoomStore {
    async fn get_room_by_discord_channel(
        ...
        with_connection(pool, move |conn| {
            use crate::db::schema_mysql::room_mappings::dsl::*;
            room_mappings
...
#[async_trait]
impl super::UserStore for MysqlUserStore {
    async fn get_user_by_discord_id(
        ...
        with_connection(pool, move |conn| {
            use crate::db::schema_mysql::user_mappings::dsl::*;
            user_mappings
...
#[async_trait]
impl super::MessageStore for MysqlMessageStore {
    async fn get_by_discord_message_id(
        ...
        with_connection(pool, move |conn| {
            use crate::db::schema_mysql::message_mappings::dsl::*;
            message_mappings
...
pub struct MysqlEmojiStore {
    pool: MysqlPool,
}

impl MysqlEmojiStore {
    pub fn new(pool: MysqlPool) -> Self {
        Self { pool }
    }
}
...
#[diesel(table_name = crate::db::schema_mysql::emoji_mappings)]
...
#[async_trait]
impl super::EmojiStore for MysqlEmojiStore {
    ...
    async fn create_emoji(&self, emoji: &EmojiMapping) -> Result<(), DatabaseError> {
        ...
        with_connection(pool, move |conn| {
            let created_at = utc_to_naive(&emoji.created_at);
            let updated_at = utc_to_naive(&emoji.updated_at);
            ...
            diesel::insert_into(crate::db::schema_mysql::emoji_mappings::table)
                .values(&new_emoji)
                .execute(conn)
                .map(|_| ())
                .map_err(|e| DatabaseError::Query(e.to_string()))
    ...
    async fn update_emoji(&self, emoji: &EmojiMapping) -> Result<(), DatabaseError> {
        ...
        with_connection(pool, move |conn| {
            ...
            diesel::update(
                crate::db::schema_mysql::emoji_mappings::table.filter(
                    crate::db::schema_mysql::emoji_mappings::discord_emoji_id
                        .eq(&emoji.discord_emoji_id),
                ),
            )
            .set(changes)
            .execute(conn)
    ...
    async fn delete_emoji(&self, discord_emoji_id: &str) -> Result<(), DatabaseError> {
        ...
        let discord_emoji_id = discord_emoji_id.to_string();
        with_connection(pool, move |conn| {
            diesel::delete(crate::db::schema_mysql::emoji_mappings::table.filter(
                crate::db::schema_mysql::emoji_mappings::discord_emoji_id.eq(discord_emoji_id),
            ))
            .execute(conn)
            .map(|_| ())
            .map_err(|e| DatabaseError::Query(e.to_string()))
