...
fn protocol_payload(domain: &str) -> ThirdPartyProtocol {
    ...
    field_types.insert(
        ...
        ThirdPartyFieldType {
            ...
            placeholder: "Discord guild id".to_string(),
        },
    );
    field_types.insert(
        ...
        ThirdPartyFieldType {
            field_type: "text".to_string(),
            placeholder: "Discord channel id".to_string(),
        },
    );
    field_types.insert(
        "userid".to_string(),
        ThirdPartyFieldType {
            field_type: "text".to_string(),
            placeholder: "Discord user id".to_string(),
        },
    );
            ...
            for mapping in mappings {
                by_guild
                    ...
                    .or_insert_with(|| ThirdPartyNetwork {
                        name: mapping.discord_guild_id.clone(),
                        protocol: "discord".to_string(),
                        fields: HashMap::from([(
                            "guild_id".to_string(),
                            mapping.discord_guild_id.clone(),
                        )]),
                    });
            }
        ...
        Ok(mappings) => {
            let locations: Vec<ThirdPartyLocation> = mappings
                ...
                .filter(|mapping| {
                    guild_filter
                        .as_ref()
                        .map(|guild| mapping.discord_guild_id == *guild)
                        .unwrap_or(true)
                        && channel_filter
                            .as_ref()
                            .map(|channel| mapping.discord_channel_id == *channel)
                            .unwrap_or(true)
                })
                .map(|mapping| ThirdPartyLocation {
                    alias: format!("#_discord_{}:{}", mapping.discord_channel_id, domain),
                    protocol: "discord".to_string(),
                    fields: HashMap::from([
                        ("guild_id".to_string(), mapping.discord_guild_id),
                        ("channel_id".to_string(), mapping.discord_channel_id),
                    ]),
                })
                .collect();
    ...
    match user_store.get_all_user_ids().await {
        Ok(user_ids) => {
            let users: Vec<ThirdPartyUser> = user_ids
                ...
                .map(|discord_user_id| ThirdPartyUser {
                    userid: format!("@_discord_{}:{}", discord_user_id, domain),
                    protocol: "discord".to_string(),
                    fields: HashMap::from([("userid".to_string(), discord_user_id)]),
                })
                .collect();
